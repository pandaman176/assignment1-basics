...
def main():
    ...
    vocab, merges = bpe.train_bpe(file_path, 1_000, special_tokens, verbose=True, use_heap=False)
    time_cost = time.time() - start_time
    start_time = time.time()
    logger.info(f"finish training bpe, {time_cost=:.2f}s")
    vocab_2, merges_2 = bpe.train_bpe(file_path, 1_000, special_tokens, verbose=True, use_heap=True)
    time_cost = time.time() - start_time
